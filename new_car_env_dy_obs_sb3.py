...
import warnings
from stable_baselines3.common.vec_env import VecEnv, VecFrameStack, vec_video_recorder
# from stable_baselines3.common.base_class import _UnvecWrapper
...
class CarEnv(gym.Env):
    ...
    def __init__(self):
        ...
        env_para_ = env_para.parameters()  # 单独定义一个对象，用于存储环境参数，simulation_car_curve_obs_new可同时调用
        ...
        # self.obs_dist_low = env_para_.obs_dist_low
        # self.obs_dist_high = env_para_.obs_dist_high   # agv与最近障碍物的距离  self.dist_vaild是障碍物的有效距离（1.5倍的距离内才需要考虑）
        ...
        self.theta_obs_car = 0.  # 车体坐标系障碍物夹角
        # reward of V
        ...
        self.reward_V = 0.  # last based_V
        self.reward_V_ = 0.  # based_V
        self.reward_V_mid = 0.  # reward_置零时存储reward_
        self.separate_reward_V = 0.  # 稀疏reward_V
        self.obs_avoid_reward = 0.
        self.delta_reward_V = 0.
        self.reward_theta = 0.  # 惩罚非目标[-pi/4,pi/4]朝向
        self.reward_stop = 0.  # 惩罚V=0
        self.acc_reward = 0.  # 惩罚加速度
        self.spin_reward = 0.  # 惩罚旋转
        self.phi_punish_positive = 0.  # 记录phi达阈值的次数，满足条件结束episode,避免采集过多不必要的交互数据
        self.phi_punish_negative = 0.
        self.goal_reward = 0.  # 目标点附近奖励 based_V在此处接近0,给定适当倍数的-based_V
        self.bound_reward = 0.  # 惩罚越界
        self.relative_dist_reward = 0.  # 相对距离奖励,替代goal_reward
        self.length_reward = 0.  # episode长度奖励
        self.total_reward = 0.  # 总奖励
        self.spin_change_reward = 0.
        self.reward_u = 0.
        self.barrier_reward = 0.  # 避障奖励
        self.crash_reward = 0.  # 碰撞奖励
        self.reach_reward = 0.
        self.negative_reward = 0.
        self.u = 0.

        self.length = 0  # episode长度

        self.dist_vaild = 0.  # AGV与最近一个障碍物的距离 与半径和 的差值（1.5倍的距离内开始考虑），判断crash的时候倍数是1，即self.barrier(默认的k=1)
        self._dist_vaild = 0.  # 上一步AGV与最近一个障碍物的距离 与半径和 的差值
        self.dist_vaild_state = 0.  ## AGV与最近一个障碍物距离
        self.obs_theta_right = 0.  # 障碍物在右侧
        self.obs_theta_left = 0.  # 障碍物在左侧
        self.obs_theta = 0.
        self.obs_theta_ = 0.
        ...
        self.dist_k = 2.5  # 障碍物距离系数

        self.state_size = 3  # demo
        self.action_size = 2.5
        self.total_reward = 0.

        self.obs_count = 0  # 进入障碍物warning区域计数
        self.stop_count = 0
    ...
    def _is_done(self):
        ...
        s = self._stop()
        t = self.sim.time > 25
        # print('i',i)                           # 调得差不多之后，可以注释掉，这个是方便查看done的原因
        # print('c',c)
        # print('ss',ss)
        # print('g',g)
        # print('s',s)
        # print('t',t)
        return i or c or g or s or t  # 从左到右 返回碰到的第一个真值    两边都是True，则and返回右边的，or返回左边的
    ...
    def _get_all_obs_theta_in_car(self):
        ...
        position_obs_in_car = rotation_matrix.dot(obs_car_dist)  # 支持多障碍物
    ...
    def _get_obs_theta_in_car(self):
        obs_car_dist = self.obstacle[:, self.obs_index] - self.sim.position  # 障碍物相对于车体的距离 转到世界坐标系的原点下
        ...
        return theta_obs_car  # 输出的仅为前向的obs方位角
    ...
    def _stop(self):
        ...
        if self.stop_count > 10 and self._get_dist(self.sim.position,
            ...
            self.reward_stop = 1.5 * self.reward_V
            self.stop_count = 0
            print('V = 0!')
        else:
            self.reward_stop = 0
    ...
    def crash(self):
        ...
        if c:
            ...
            self.crash_reward = 1e1 * self.reward_V
        return c
    ...
    def barrier(self, x, x_so, obs_num=-1, k=1.):  # Returns the distance between the robot and the obstacle within the collision threshold
        ...
        if obs_num == -1:  # 如果没有传入obs_num，那么就是所有的障碍物都遍历一遍 对应于 def crash(self):
            r = obs_r
        else:
            r = obs_r[obs_num]  # 接收自定义 obs_r，可以跟送进来的x_so一样 通过obs_num来索引对应obs的半径
        R = self.R  # AGV半径
        gain = 1
        xi = np.array(1e-12)
        ...
        if nbObs <= 2:
            x_obs = x_so
            a = self._get_dist(x, x_obs)  # 欧式距离   [:, np.newaxis]
    ...
    def _get_real_reward(self):
        ...
        if self.sim.v < 0. and self.dist_vaild > 1e-4:  #
            self.negative_reward = 8 * self.reward_V_
        else:
            self.negative_reward = 0
    ...
    def _get_observation(self):
        ...
        self.dx_u, self.u, Vx, Vdot, self.V = ds_stabilizer(self.get_virtual_position(), self.dist_vaild,
                                                            ...
                                                            # whether use u_modified
                                                            self.sim.theta, self.sim.phi,
                                                            option, self.Vxf,
                                                            self.gparameter, 0,
                                                            dx_rl=self.dx_car, x_obs=self.obstacle,
                                                            r=self.r)  # RL=True 表示单个agent     obs_pos=self.obs_pos_embeding,
        ...
        # Orientation error
        theta_to_goal = self._get_goal_theta_in_car()  # 车体坐标系下的角度    acrctan2返回值范围是(-pi, pi)
        if theta_to_goal < 0:
    ...
    def step(self, action):
        ...
        self.reward_V = self.reward_V_mid  # 没用


        self.obs_theta = self.obs_theta_  # 区别于delta_v需要保留非置零的v来进行比较    而这里不能取判定为0后的原有值，因为这会包含>1e-4的情况
        ...
        theta_last = self.sim.theta  # sim.theta ~ [-pi, pi]           因为是在step之前，因此是上一时刻的theta
        if theta_last < 0:
            theta_last += 2 * np.pi  # theta_last ~ [0, 2pi]


        dxu_theta = max(theta_last, dxu_rad) - min(theta_last, dxu_rad)  # u和theta角度之间的相对差值
        # dx_theta = max(theta_last, dx_rad) - min(theta_last, dx_rad)        # dx和theta角度之间的相对差值   判断a的方向
        ...
        du_theta_rad = dxu_rad - theta_last  # 换算到[0,2*pi]之后的dx和du 角度之间的相对差值
        if du_theta_rad < 0:
