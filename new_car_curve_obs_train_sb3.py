...
from simulation_car_curve_obs_new_sb3 import Simulation as sim
import matplotlib
...
if __name__=="__main__":
    ...
    if run_mode == 'train':
        ...
        if method == "SAC":
            ...
            policy_kwargs = dict(net_arch=dict(pi=[256, 256,  128], qf=[256, 256,  128]))  # [256, 256, 128] 直接都取阈值了
            ...
            SAC_callback = SaveOnBestTrainingRewardCallback(check_freq=10000, log_dir=log_dir)  # 因为初始点过近导致简单到了目标点，reward也会很大（接近0），此时模型不一定是最好的，因此暂时都保存下来
